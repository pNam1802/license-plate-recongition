...
Thao tác:
    ...
    Space         — chuyển sang frame kế tiếp (tìm góc nhìn tốt hơn)
    Q             — thoát không lưu
"""
...
def _draw(frame: np.ndarray) -> np.ndarray:
    ...
    if len(pts) >= 3:
        ...
        cv2.polylines(out, [poly], isClosed=True, color=(0, 200, 255), thickness=2)

    # Đường nối điểm cuối → con trỏ (chỉ có ý nghĩa visual)
    if len(pts) >= 2:
        for i in range(len(pts) - 1):
            cv2.line(out, pts[i], pts[i + 1], (0, 200, 255), 2)
...
def main():
    parser = argparse.ArgumentParser(description="Vẽ Zone of Interest cho pipeline nhận diện biển số")
